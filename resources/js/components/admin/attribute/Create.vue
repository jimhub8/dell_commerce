...
<v-layout row justify-center>
    <v-dialog v-model="dialog" persistent max-width="500px">
        <v-card>
            ...
            <v-card-text>
                <v-flex sm12>
                    <v-text-field v-model="form.name" color="blue darken-2" label="Attribute" required></v-text-field>
                    <small class="has-text-danger" v-if="errors.name">{{ errors.name[0] }}</small>
                </v-flex>
                <v-flex sm12>
                    <label for="">Attribute</label>
                    <el-select v-model="form.frontend_type" clearable filterable placeholder="Select attribute">
