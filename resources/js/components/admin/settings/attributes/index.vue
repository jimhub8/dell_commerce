            ...
            <v-layout justify-center align-center v-show="!loader">
                <v-card>
                    <v-card-title>
                        ...
                        <!-- <v-spacer></v-spacer> -->
                        <v-tooltip bottom>
                            <v-btn slot="activator" icon class="mx-0" @click="getAtributest">
                    ...
                    <v-data-table :headers="headers" :items="attributes" class="elevation-1" :search="search" :loading="loading">
                        ...
                        <template slot="items" slot-scope="props">
                            ...
                            <td class="justify-center layout px-0">
                                <v-tooltip bottom>
                                    <v-btn slot="activator" icon class="mx-0" @click="editAttribute(props.item)">
                                        <v-icon small color="blue darken-2">edit</v-icon>
                                    </v-btn>
                                    <span>Edit {{ props.item.attribute }}</span>
                                </v-tooltip>
                            </td>
                        </template>
...
export default {
    ...
    methods: {
        ...
        editAttribute(attribute) {
            eventBus.$emit('editAttributeEvent', attribute)
        },
        getAtributest() {
            this.loader = true;
            axios
    ...
    //     next(vm => {
    //         if (vm.user.can['view attributes']) {
    //             next();
    //         } else {
    //             next('/unauthorized');
    //         }
